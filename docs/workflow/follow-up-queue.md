...
- `앱 제품 우선 전환 묶음` — 활성. 첫 진입을 설명형 랜딩이 아니라 앱 화면으로 전환했고, PWA manifest 기초, TWA 체크리스트, signing key·assetlinks 준비를 완료했다. 현재 티켓은 `docs/workflow/work-plan.md`의 `Android TWA 프로젝트 생성 1차`다.
