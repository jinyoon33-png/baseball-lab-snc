...
- 현재 상태: 커밋 `7bb8ce8`, `47aa955` 원격 반영 확인. 공개 privacy 문구는 반영됐으나 CSP가 기존 정적 `_headers`로 응답해 root `functions/_middleware.js`가 실행되지 않음. Cloudflare Pages 프로젝트 루트가 `site/`로 보이므로 미들웨어 위치를 `site/functions/_middleware.js`로 보정 중.

2. 대상 파일
- 커밋 대상: `site/functions/_middleware.js`, `site/*.html`, `site/robots.txt`, `site/sitemap.xml`, `docs/workflow/work-plan.md`, `docs/workflow/work-plan-archive.md`
...
- `site/functions/_middleware.js`가 Cloudflare Pages 프로젝트 루트 기준 Functions 디렉터리에 있고 root `functions` 잔재가 없는지 확인한다.
...
- `node --check site/app.js && node --check site/data.js && node --check site/functions/_middleware.js`
