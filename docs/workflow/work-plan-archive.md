...
- Owner: 총괄 Codex.
- Finding: `site/index.html` 신규 등록/편집 키 라벨에 `필수` 배지가 없고, `site/app.js`도 키 빈 값을 `0`으로 fallback해 허용 중.
- Decision: 단순 라벨 보정이 아니라 키 빈 값 차단과 `0` 우회 차단까지 함께 처리해야 UI와 검증이 일치한다.
- Next: `신규 선수 등록 키 필수 표시·검증 보정 1차`.
