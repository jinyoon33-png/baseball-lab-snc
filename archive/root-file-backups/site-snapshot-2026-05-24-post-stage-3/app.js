...
function calculateLiveWorkload() {
    ...
    if (rpeInput !== '' && pitchCountInput !== '') {
        ...
        if (workload === 0) {
            displayEl.style.background = 'var(--bg-color)';
            displayEl.style.color = 'var(--text-muted)';
            displayEl.style.border = '1px solid var(--border)';
            ...
            displayEl.style.background = 'rgba(45, 106, 79, 0.1)';
            displayEl.style.color = 'var(--primary)';
            displayEl.style.border = '1px solid rgba(45, 106, 79, 0.2)';
...
function savePerformance() {
    ...
    if (pType === '타자') {
        ...
        if (maxImproved || rpmImproved) {
            ...
            p.isUpgraded = true;
            p.upgradeMsg = `축하합니다! 퍼포먼스 향상 (${improvedParts.join(', ')})`;
        } else {
            p.isUpgraded = false;
            p.upgradeMsg = `꾸준함이 정답입니다! 다음 측정까지 파이팅!`;
        }
    ...
    radarChartInstance = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: '현재 능력치 (1~5단계)',
                data: scoreData,
                backgroundColor: 'rgba(45, 106, 79, 0.2)', borderColor: getCssVar('--primary'),
    ...
    if (pType === '타자') {
        ...
        trendChartInstance = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [
                    { label: '타구속도 (km/h)', data: exitVeloData, borderColor: getCssVar('--chart-amber'), backgroundColor: 'rgba(245,158,11,0.1)', yAxisID: 'y', tension: 0.3, fill: true },
                    { label: '배트스피드 (km/h)', data: batSpeedData, borderColor: getCssVar('--chart-red'), backgroundColor: 'rgba(239,68,68,0.1)', yAxisID: 'y', tension: 0.3, fill: true }
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
        ...
        trendChartInstance = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [
                    { label: '최고 구속 (km/h)', data: maxVeloData, borderColor: getCssVar('--chart-red'), backgroundColor: 'rgba(239,68,68,0.1)', yAxisID: 'y', tension: 0.3, fill: true },
                    { label: '평균 구속 (km/h)', data: avgVeloData, borderColor: getCssVar('--chart-green'), backgroundColor: 'rgba(16,185,129,0.1)', yAxisID: 'y', tension: 0.3, fill: true },
                    { label: 'RPM', data: rpmData, borderColor: getCssVar('--primary'), backgroundColor: 'rgba(45,106,79,0.1)', yAxisID: 'y1', tension: 0.3, fill: true }
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
...
function renderACWR(p) {
    ...
    if (!isReady) {
        ...
        document.getElementById('acwrStatus').style.backgroundColor = 'var(--bg-color)';
        document.getElementById('acwrStatus').style.color = 'var(--text-muted)';
        return;
    }
    ...
    if (acwr === 0) {
        ...
        statusEl.style.backgroundColor = 'var(--bg-color)';
        statusEl.style.color = 'var(--text-muted)';
    } else if (acwr >= 0.8 && acwr <= 1.3) {
        statusEl.innerText = '안전 영역 (권장 부하 범위)';
        statusEl.style.backgroundColor = 'var(--success-light)';
        statusEl.style.color = 'var(--success)';
    } else if (acwr > 1.3 && acwr <= 1.5) {
        statusEl.innerText = '주의 영역 (부상 위험 증가)';
        statusEl.style.backgroundColor = 'var(--warning-light)';
        ...
        statusEl.style.backgroundColor = 'var(--info-light)';
...
function renderWeeklyCalendar(p) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        html += `
            <div class="calendar-cell ${statusClass}" data-calendar-cell-action="open-card">
                <div class="cal-day-label">Day ${i + 1}</div>
                <div class="cal-date">${escapeHTML(dateStr)}(${escapeHTML(dayName)})</div>
                <div class="cal-status-badge">${escapeHTML(statusText)}</div>
                ${isCompleted ? `<div class="cal-workload">WL: ${escapeHTML(String(p.dailyCompletion[i].workload ?? ''))}</div>` : ''}
            </div>
        `;
    }
    html += `</div>`;
...
function renderTeamDashboard() {
    ...
    if (queueItems.length === 0) {
        ...
        actionQueue.innerHTML = queueItems.map(({ p, risk }) => {
            ...
            if (risk.acwrDanger) reasons.push('<span class="aq-tag aq-danger">ACWR 위험</span>');
            if (risk.acwrCaution) reasons.push('<span class="aq-tag aq-warning">ACWR 주의</span>');
...
function renderBadgesAndSchedule(p) {
    ...
    if (acwr > 1.5) {
        ...
        badgesHtml += `<div class="badge badge-warning">ACWR 주의 (${acwr.toFixed(2)}): 상체 운동 볼륨이 50% 감소합니다.</div>`;
    }
    ...
    if (summarySection) {
        if (isPreviewMode) {
            ...
            const acwrStatusText = acwr > 1.5 ? 'ACWR 위험' : acwr > 1.3 ? 'ACWR 주의' : 'ACWR 정상';
...
function renderSwapOption(altName, scoreResult, announcedName) {
    ...
    return `
        <div class="swap-option" data-name="${escapeHTML(altName)}">
